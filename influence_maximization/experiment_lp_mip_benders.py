...
def gen_ioweight(g, netname):
    ...
    # 2     [(1,0.3) (3,0.4) ...]
    # ...
    ipath = './data/%s-GraphML/iweights_%s.pickle' % (netname, netname)
    opath = './data/%s-GraphML/oweights_%s.pickle' % (netname, netname)
    if os.path.exists(ipath) and os.path.exists(opath):
        inweights   = pickle.load(open(ipath))
        outweights  = pickle.load(open(opath))
    else:
        ...
        pickle.dump(list(inweights), open(ipath, 'wb'))
        pickle.dump(list(outweights), open(opath, 'wb'))
...
def get_result_base(g, size, source, target, weight):
...
def run_weight():
    ...
    pname = gen_graph()
    g, size, source, target, weight = load_graph(pname)
    get_result_base(g, size, source, target, weight)
    print 'Done! seed set selected by MAX-weighted-degree'


def run_benders(netname):
    print 'Selecte seed set by approx-Benders'
    pname = gen_graph()
    g, size, source, target, weight = load_graph(pname)
    inweights, outweights = gen_ioweight(g, netname)
    sepflag = 0
